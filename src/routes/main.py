...
def add_substation():
    if not current_user.is_inspector():
        ...
        return redirect(url_for("main.dashboard"))
    substations = Substation.query.all() # This line is redundant, but harmless if it remains
...
def metrics():
    ...
    # Weekly Trend
    weekly_metrics = ReliabilityMetric.query.filter(ReliabilityMetric.date >= (date.today() - timedelta(weeks=12)))
    weekly_metrics = weekly_metrics.order_by(ReliabilityMetric.date).all()
    
    # Monthly Trend
    monthly_metrics = db.session.query(
        func.to_char(ReliabilityMetric.date, "YYYY-MM").label("month"), # Fix: Changed strftime to to_char for PostgreSQL compatibility
    ...
    ).filter(ReliabilityMetric.date >= (date.today() - timedelta(days=365)))
    monthly_metrics = monthly_metrics.group_by("month").order_by("month").all()

    # Yearly Trend
    yearly_metrics = db.session.query(
        func.to_char(ReliabilityMetric.date, "YYYY").label("year"), # Fix: Changed strftime to to_char for PostgreSQL compatibility
        func.avg(ReliabilityMetric.reliability_score).label("avg_reliability"),
        func.avg(ReliabilityMetric.testing_compliance).label("avg_testing_compliance"),
        func.avg(ReliabilityMetric.inspection_compliance).label("avg_inspection_compliance"),
        func.avg(ReliabilityMetric.coverage_ratio).label("avg_coverage_ratio"),
        func.avg(ReliabilityMetric.effective_reliability).label("avg_effective_reliability")
    ).filter(ReliabilityMetric.date >= (date.today() - timedelta(days=365*5)))
    yearly_metrics = yearly_metrics.group_by("year").order_by("year").all()
